...
class RetrievalClient:
    ...
    def retrieve(self, key: CacheKey, host: str, port: int = 80) -> None:
        """Retrieve a block from another peer"""
        conn = http.client.HTTPConnection(host, port)
        ranges = [pccrr.Range(index=key.block_index, count=1)]
        req = pccrr.MsgGetBlks(segment_id=key.segment_id,
        ...
        self.cache[key].msg = rsp
        logger.debug("%s: cached", key)
